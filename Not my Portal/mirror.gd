...
func get_mirror_size() -> Vector2:
	...
	if(interface):
		return interface.get_render_target_size() * 1.0
	else:
		return get_viewport().size * 1.0
...
func render_view(p_interface: Object, p_view_index: int, p_cam: Camera3D) -> void:
	...
	if p_interface is XRInterface:
		...
		tx = (origin.get_node("HMD") as XRCamera3D).global_transform
		tx.origin =+ tx.basis.x*0.0063*((p_view_index-0.5)*2)
	...
	if use_screenspace:
		...
		p_cam.set("projection",  Projection(Transform3D.FLIP_X) * proj *  Projection(Transform3D.FLIP_X))

	if !use_screenspace:
		var px = Projection(Vector4.ZERO, Vector4.ZERO, Vector4.ZERO, Vector4.ZERO)
		p_cam.set("projection", px)
